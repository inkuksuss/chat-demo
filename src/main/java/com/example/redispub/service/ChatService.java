...
public class ChatService {
    ...
    public void joinRoom(Long memberId, Long roomId) {
        ...
        chatRepository.save(new RoomInfo(roomId, memberId));

        ChatDto<RoomInfoDto> chatDto = new ChatDto<>();
        chatDto.setMemberId(memberId);
        chatDto.setRoomId(roomId);
        chatDto.setActionType(ActionType.ROOM_JOIN);
        chatDto.setData(this.getRoomParticipationInfomation(roomId, findMemberList));

        redisTemplate.convertAndSend("/actions", chatDto);
    }

    private RoomInfoDto getRoomParticipationInfomation(Long roomId, List<RoomMapping> findMemberList) {
        List<Long> totalMemberIdList = findMemberList.stream()
                .map(RoomMapping::getMember)
                .map(Member::getId)
                .toList();

        List<Long> currentMemberIdList = chatRepository.findByRoomId(roomId).stream()
